...
contract user_Token is StandardToken {
    ...
    /*个人信息录入*/
    uint userLength = 0;
    uint voterSize = 0;//股东投票人数
    uint dsSize = 0;//董事投票人数
    struct userMsg {
      string name;
      string surname;
      string sex;
    ...
    mapping(address => bool) userHaveVote; //股东是否投票  
    mapping(address => bool) userDSHaveVote; //董事是否投票   
   ...
   /*计划列表*/
    ...
    struct planModule {
      ...
      uint successNum;
      uint failNum;
      mapping(address => bool) dsIsVote;
    }
    
     mapping(uint => planModule) private planList;
    
     event createPlan( 
        address myAddress,
        string indexed planName,
        string indexed content
    );
    /*创建董事决议*/
    function addPlan(address _myAddress,string _planName ,uint _num ,uint _firstNum,uint _dateType ,uint _dateSize,string _content) public {
        planList[planLength] = planModule(0,_myAddress,_content,_planName,_num,_firstNum,_dateType,_dateSize,now,0,0);
        planList[planLength].dsIsVote[msg.sender] = false;
        emit createPlan(_myAddress,_planName,_content);
        planLength = planLength.add(1);
    }
    
    function setPlan(uint code,bool _isVote) public returns (bool){
        require(now < planList[code].createTime + duration * 1 hours);
        require(planList[code].state == 0);
        require(!planList[code].dsIsVote[msg.sender]);
        require(getPosition(msg.sender) == 1 || getPosition(msg.sender) == 3);
        if(_isVote){
            planList[code].successNum =  planList[code].successNum.add(1);
            if( planList[code].successNum > dsSize / 2){
                planList[code].state == 1;
            }
        }else{
             planList[code].failNum =  planList[code].failNum.add(1);
             if( planList[code].failNum >= dsSize / 2){
                planList[code].state == 2;
             }
        }
    }
    
    function setDSVoteList(uint code,bool _isVote) public returns(bool){
        require(now < voteList[code].createTime + duration * 1 hours);
        require(voteList[code].state == 0);
        require(!voteList[code].userIsVote[msg.sender]);
        require(userHaveVote[msg.sender]);
